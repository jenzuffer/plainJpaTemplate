...
//EntityManagerFactory emf = Persistence.createEntityManagerFactory("pu-test", null);
    ...
    public void getCarsByMakeTest() {
        List<Car> carsByMake = facade.getCarsByMake("Volve");
        Assert.assertEquals("Volve", carsByMake.get(0).getMake());
    }

    @Test
    public void getExistingID() {
        int carID = facade.getcarsID();
        Assert.assertEquals(0, carID);
    }
    ...
    public void DeleteCarByIDTest() {
        ...
        int newSize = facade.deleteCarByID(allCars.get(0).getId());
